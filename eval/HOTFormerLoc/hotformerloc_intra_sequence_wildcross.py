...
import hotformerloc_utils

pd.options.display.float_format = "{:,.2f}".format

def model_factory(config, model_config):
    params = hotformerloc_utils.load_params(config, model_config)
    return params, hotformerloc_utils.load_model(params)

class Evaluator:
    def __init__(self, model, pickle_file, split_idx, dataset_root, save_dir, params, debug=False):
        self.pos_thresh = 3.0 
    ...
    @torch.no_grad()
    def get_latent_vectors(self, env_data, seq_name, params, debug):
        # Load vectors from pickle file if they have already been stored in save dir
        vectors_save_path = os.path.join(self.save_dir, f'{seq_name}_vectors_split_{args.split_idx}.pickle')
        if os.path.exists(vectors_save_path):
            vectors = pickle.load(open(vectors_save_path, 'rb'))
        else:
            vectors = hotformerloc_utils.get_latent_vectors(self.model, env_data, params)
...
if __name__ == '__main__':
    ...
    device = "cuda"
    params, model = model_factory(args.config, args.model_config)
    assert os.path.exists(args.ckpt), "Cannot open network weights: {}".format(args.ckpt)
    print("Loading weights: {}".format(args.ckpt))
    try:  # Checkpoint file
        state = torch.load(args.ckpt)
        model.load_state_dict(state["model_state_dict"])
    except KeyError or RuntimeError:  # Weights only
